...
contract('CalileaToken', async (accounts) => {
  var tokenInstance;
  ...
  it('initializes the contract with the correct values', async() => {
    ...
    assert.equal(name, 'Calilea Token', 'has the correct name');

    let symbol = await tokenInstance.symbol();
    assert.equal(symbol, 'CAL', 'has the correct symbol');
  ...
  it('approves tokens for delegated transfer', async() => {
    let tokenInstance = await CalileaToken.deployed();
